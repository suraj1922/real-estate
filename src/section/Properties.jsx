  ...
  return (
    <div>
      <div className={`${darkMode ? 'dark bg-black' : 'light bg-transparent'}`}>
        <section id='properties' className='lg:w-[90%] m-auto lg:px-20 px-6 py-20 w-full flex flex-col justify-center items-start gap-4'>
          <div className='flex flex-col justify-center items-start gap-4'>
            <h1 data-aos="zoom-in" className={`text-red-500 ${darkMode ? 'dark:text-white' : ''}`}>PROPERTIES</h1>
            <h1 data-aos="zoom-in" className={`text-4xl font-semibold ${darkMode ? 'dark:text-white' : 'text-black'}`}> Explore the latest</h1>
          </div>
          ...
          <div id="grid-box" className='w-full grid lg:grid-cols-3 grid-cols-1 justify-center items-center gap-8'>
            {property.map((item, index) => (
              <div data-aos="zoom-in" data-aos-delay="200" key={index} className={`bg-white ${darkMode ? 'dark:bg-gray-800' : ''} rounded-xl w-full`}>
              <div id='image-box' className='bg-cover bg-center h-[250px] rounded-xl p-4 flex flex-col justify-between item-end' style={{ backgroundImage: `url(${item.images})` }}>
                ...
                <div className='px-6 py-3 flex flex-col justify-center items-start gap-2 w-full'>
                  <h1 className={`text-xl font-semibold ${darkMode ? 'dark:text-white' : 'text-black'}`}>{item.name}</h1>
                  <h1 className={`text-2xl font-bold ${darkMode ? 'dark:text-white' : 'text-red-600'}`}>{item.price}</h1>
                  <p className={`${darkMode ? 'dark:text-white' : ''}`}>{item.about}</p>
                  <div id='icons' className='flex justify-center items-start gap-4'>
                    <div className='flex justify-center items-center gap-2'>
                      <FaBath className='size-5 text-red-400' />
                      <h1 className={`${darkMode ? 'dark:text-white' : ''}`}>{item.bath}</h1>
                    </div>
                    <div className='flex justify-center items-center gap-2'>
                      <FaBed className='size-5 text-red-400' />
                      <h1 className={`${darkMode ? 'dark:text-white' : ''}`}>{item.bed}</h1>
                    </div>
                    <div className='flex justify-center items-center gap-2'>
                      <MdSpaceDashboard className='size-5 text-red-400' />
                      <h1 className={`${darkMode ? 'dark:text-white' : ''}`}>{item.area}</h1>
                    </div>
                  </div>
                  ...
                  <div id='owner-info' className='flex justify-between items-center w-full mt-2'>
                    <div className='flex justify-center items-center gap-2'>
                      ...
                      <h1 className={`${darkMode ? 'dark:text-white' : ''}`}>{item.owner}</h1>
                    </div>
                    <div className='flex justify-center items-center gap-4'>
